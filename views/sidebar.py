...
def ensure_b1_to_b5_loaded(DATA_DIR):
    """🚀 背景自動補載機制：當搜尋時發現數據缺失，自動觸發後台同步引擎"""
...
def robust_search_engine(df, query):
    ...
    if col_id: df[col_id] = df[col_id].astype(str).str.replace(r'\.0$', '', regex=True).str.strip()
    if col_name: df[col_name] = df[col_name].astype(str).str.strip()

    exact_mask = pd.Series(False, index=df.index)
    if col_id: exact_mask = exact_mask | (df[col_id] == query)
    if col_name: exact_mask = exact_mask | (df[col_name].str.lower() == query.lower())

    exact_result = df[exact_mask]
    if not exact_result.empty: return exact_result

    partial_mask = pd.Series(False, index=df.index)
    if col_id: partial_mask = partial_mask | df[col_id].str.contains(query, na=False)
    if col_name: partial_mask = partial_mask | df[col_name].str.contains(query, na=False, case=False)
