...
def render(STOCK_DICT=None):
    ...
    scan_tab1, scan_tab2 = st.tabs(["依主力(Top15)買超張數排行", "依股價乖離率(吃豆腐)排行"])
    
    with scan_tab1:
        # 讀取 HF 上面的 Parquet，檔名需要完全吻合您上傳的名稱 (有雙底線)
    ...
    calc_days = int(fetch_text_from_hf("momentum_meta.txt"))
    ...
    if not df_momentum.empty:
        ...
        def render_momentum_tab(df, prefix, rank_col_name):
            ...
            if '名次變化' in disp_df.columns: styled = styled.map(color_chg, subset=['名次變化'])
            if f'{prefix}Δ' in disp_df.columns: 
