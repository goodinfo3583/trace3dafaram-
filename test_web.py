...
# 🛠️ 強化搜尋函數：增加欄位強制轉換與深度清理
def safe_search(df, query):
    if df is None or df.empty:
        return pd.DataFrame()
    
    # 清洗：確保代號為字串且去除前後空格，以便精準匹配
    df_temp = df.copy()
    if '股票代號' in df_temp.columns:
...
if search_query:
    ...
    if q_txt.empty:
        st.warning(f"⚠️ 找不到與 '{search_query}' 相關資料。")
    else:
        st.write(f"### 🎯 綜合診斷標的：{q_txt.iloc[0]['股票名稱']} ({q_txt.iloc[0]['股票代號']})")
        ...
        # 這裡的變數名稱對應您區塊2-1~2-4的原始資料變數
        q_2_1 = safe_search(csv_foreign_deal, search_query) # 外資5日佔成交
        q_2_2 = safe_search(csv_it_deal, search_query)      # 投信5日佔成交
        q_2_3 = safe_search(csv_foreign_stock, search_query)# 外資5日佔發行
        q_2_4 = safe_search(csv_it_stock, search_query)     # 投信5日佔發行

        c1, c2 = st.columns(2)
        with c1:
            st.write("📊 **區塊 2-1：外資 5 日淨買佔成交量**")
            st.dataframe(q_2_1 if not q_2_1.empty else pd.DataFrame({"訊息": ["無資料"]}), use_container_width=True)
            st.write("🔒 **區塊 2-3：外資 5 日淨買佔發行量**")
            st.dataframe(q_2_3 if not q_2_3.empty else pd.DataFrame({"訊息": ["無資料"]}), use_container_width=True)
                
        with c2:
            st.write("🥦 **區塊 2-2：投信 5 日淨買佔成交量**")
            st.dataframe(q_2_2 if not q_2_2.empty else pd.DataFrame({"訊息": ["無資料"]}), use_container_width=True)
            st.write("💎 **區塊 2-4：投信 5 日淨買佔發行量**")
            st.dataframe(q_2_4 if not q_2_4.empty else pd.DataFrame({"訊息": ["無資料"]}), use_container_width=True)
        ...
        with c3_1:
            st.write("🌐 **外資最新日連買**")
            st.dataframe(safe_search(live_fo_day, search_query) if not live_fo_day.empty else "無資料", use_container_width=True)
            st.write("🌐 **外資最新週連買**")
            st.dataframe(safe_search(live_fo_wk, search_query) if not live_fo_wk.empty else "無資料", use_container_width=True)
        with c3_2:
            st.write("🏦 **投信最新日連買**")
            st.dataframe(safe_search(live_it_day, search_query) if not live_it_day.empty else "無資料", use_container_width=True)
            st.write("🏦 **投信最新週連買**")
            st.dataframe(safe_search(live_it_wk, search_query) if not live_it_wk.empty else "無資料", use_container_width=True)
# ==========================================
# 🧭 側邊欄導航 (無感互動+視覺特效版)
# ==========================================
st.sidebar.markdown("---")
