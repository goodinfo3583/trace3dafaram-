...
if (!parentDoc.getElementById('custom-sticky-header')) {
    ...
    headerDiv.innerHTML = `
        <div class="disclaimer-bar">
            ...
            <div class="disclaimer-item"><span class="disclaimer-title">隱私政策</span><div class="disclaimer-content"><b>1. 蒐集目的與範圍：</b><br>本平台依個資法蒐集您的識別資料僅供維持系統安全與優化服務使用。<br><b>2. 資料利用：</b><br>您的資料絕不向第三方洩露。<br><b>3. 資料刪除：</b><br>您可透過「聯絡我們」請求刪除資料。<br><b>4. 政策修訂：</b><br>本站保留修改政策之權利，繼續使用即視為同意。</div></div>
...
# ======================================================
# ======================================================
# 分頁3 - 總經導航 (🚀 6重備援與強制破除快取版 _v2)
# ======================================================
@st.cache_data(ttl=300) 
def fetch_macro_indicators_v2(): # 💡 注意：名稱改為 _v2 強制系統重新抓取
    import requests
    import re
    import datetime
    import yfinance as yf
    ...
    if data["vix"]["value"] is None:
        try:
            ...
            if len(hist) >= 2:
                data["vix"]["value"] = float(hist['Close'].iloc[-1])
                data["vix"]["pct"] = (float(hist['Close'].iloc[-1]) - float(hist['Close'].iloc[-2])) / float(hist['Close'].iloc[-2]) * 100
        except: pass


    # --- 2. 🇹🇼 台股 VIX (VIXTWN) 終極 6 重備援 ---
    
    # 🌟 來源 1: Yahoo Finance TW (拆解底層 JSON 狀態)
    try:
        res_yw = requests.get("https://tw.stock.yahoo.com/quote/%5EVIXTWN", headers=headers, timeout=5)
        match_p = re.search(r'"regularMarketPrice":\{"raw":([\d\.]+)', res_yw.text)
        match_prev = re.search(r'"regularMarketPreviousClose":\{"raw":([\d\.]+)', res_yw.text)
        if match_p:
            v = float(match_p.group(1))
            if 10.0 <= v <= 150.0:
                data["vixtwn"]["value"] = v
                if match_prev and float(match_prev.group(1)) > 0:
                    data["vixtwn"]["pct"] = (v - float(match_prev.group(1))) / float(match_prev.group(1)) * 100
                else:
                    data["vixtwn"]["pct"] = 0.0
    except: pass
    ...
    if data["vixtwn"]["value"] is None:
        try:
            res_sq = requests.get("https://www.stockq.org/index/VIXTW.php", headers=headers, timeout=5)
    ...
    if data["vixtwn"]["value"] is None:
        try:
            res_wg = requests.get("https://www.wantgoo.com/global/vixtwn", headers=headers, timeout=5)
            match = re.search(r'class="price"[^>]*>([\d\.]+)</', res_wg.text)
            if match:
                v = float(match.group(1))
                if 10.0 <= v <= 150.0:
                    data["vixtwn"]["value"] = v
    ...
    # 🌟 來源 5: FinMind API (拉長回溯至 30 天，確保跨越長假也能抓到最後一筆)
    if data["vixtwn"]["value"] is None:
        try:
            ...
            if res_fm.status_code == 200:
                ...
                if len(fm_data) > 0:
                    v = float(fm_data[-1].get("VIX"))
                    if 10.0 <= v <= 150.0:
                        data["vixtwn"]["value"] = v
                        if len(fm_data) >= 2:
                            p_v = float(fm_data[-2].get("VIX"))
                            data["vixtwn"]["pct"] = (v - p_v) / p_v * 100
                        else:
                            data["vixtwn"]["pct"] = 0.0
        except: pass

    # 🌟 來源 6: yfinance (嘗試隱藏代號)
    if data["vixtwn"]["value"] is None:
        try:
            hist = yf.Ticker("^VIXTWN").history(period="5d")
            if len(hist) >= 2:
                v = float(hist['Close'].iloc[-1])
                if 10.0 <= v <= 150.0:
                    data["vixtwn"]["value"] = v
                    p_v = float(hist['Close'].iloc[-2])
                    data["vixtwn"]["pct"] = (v - p_v) / p_v * 100
        except: pass
...
#這裡原本有系統綜合假評分
    ...
    if not search_query:
        ...
        with tab3:
            macro_data = fetch_macro_indicators_v2() # 🚀 改為呼叫 V2 版本
