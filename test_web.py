...
if (!parentDoc.getElementById('custom-sticky-header')) {
    ...
    headerDiv.innerHTML = `
        ...
        <div class="nav-btn-container" id="nav-btn-container">
            ...
            <a href="#" data-target="NavToPool" class="nav-text-link internal-nav"><img src="app/static/jewllove.png" class="nav-icon" alt="icon">觀察名單</a><span class="nav-divider">|</span>
...
def show_news_page():
    ...
    with col2:
        search_query = st.text_input("🔍 搜尋新聞標題、關鍵字或股票代號")
    ...
    st.caption(f"<div style='text-align: center; color: #94A3B8; margin-bottom: 20px;'>共有 {total_items} 則新聞，目前顯示第 {start_item} 到 {end_item} 則，千萬別僅憑新聞買賣操作</div>", unsafe_allow_html=True)
...
if current_page in ["all", "b1"]:
    ...
    st.write("")
    st.info("今日上榜是指5/20/60/120日ΔChange累計變化維持在法人持股變化前200名，△是單日的法人持股增減(如果最新基準日未進前200榜，△會直接以歸0計算)；我們可以試著短線與長線一起觀察。")
...
###########
                ...
                if selected_view == "🔹 今日最新排行" or selected_view == "今日最新排行":
                    ...
                    st.write("---")
                    st.markdown("### 🧩 觀察名單中法人的資金聚落")
                    st.caption("我們將上方的觀察名單透過產業類別面積大小直接表示出來，或許可以找找看目前法人口袋觀察名單集中在哪些產業 ,這裡我們排除了 ETF 與債券。")
                        ...
                        if not treemap_pool_df.empty:
                            ...
                            fig.update_traces(
                                ...
                                marker=dict(
                                    ...
                                    pad=dict(t=35, l=10, r=10, b=10)     # 增加產業標題與股票名稱之間的距離
                                ),
                                hovertemplate=(
                                    '<b>%{label}</b><br>'
                                    '股票代號: %{customdata[0]}<br>'
