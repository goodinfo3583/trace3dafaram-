...
<div class="course-title"><img src="app/static/icon-course1.png" class="course-icon">Lv 2. 股市基本架構與名詞解析 (未開放)</div>
...
<tr><td style="color:#EF4444; font-weight:bold;">🚨 多殺多風險</td><td>股價↓↓ / 融資↓↓ / 融券→↓ / 借券→↓</td><td style="text-align: left;">股價下跌導致融資退場，可能出現槓桿賣壓</td></tr>
        ...
        # =========================
        # 🔗 核心隱藏按鈕 (高度獨立命名，徹底避免與其他模組的 Key 衝突)
        # =========================
        if st.button("BtnCloseCourseNPC", key="st_btn_course_npc_close_x99"):
            st.session_state['show_course_npc'] = False
            st.session_state['course_view'] = 'list'
            st.rerun()
            
        if st.button("BtnOpenCourse1", key="st_btn_course_npc_open_1_x99"):
            st.session_state['course_view'] = 'detail_1'
            st.rerun()
            
        if st.button("BtnOpenCourse4", key="st_btn_course_npc_open_4_x99"):
            st.session_state['course_view'] = 'detail_4'
            st.rerun()

        if st.button("BtnOpenCourse7", key="st_btn_course_npc_open_7_x99"):
            st.session_state['course_view'] = 'detail_7'
            st.rerun()
            
        if st.button("BtnBackToCourseList", key="st_btn_course_npc_back_x99"):
            st.session_state['course_view'] = 'list'
            st.rerun()
            
        # 💡 JS 綁定按鈕與隱形引擎 (對應上述新的獨立名稱)
        bind_js = """<script>
setInterval(() => {
const doc = window.parent.document;
if (!doc) return;
...
const btnClose = stBtns.find(b => b.textContent.includes('BtnCloseCourseNPC'));
const btnOpen1 = stBtns.find(b => b.textContent.includes('BtnOpenCourse1'));
const btnOpen4 = stBtns.find(b => b.textContent.includes('BtnOpenCourse4'));
const btnOpen7 = stBtns.find(b => b.textContent.includes('BtnOpenCourse7'));
const btnBack = stBtns.find(b => b.textContent.trim() === 'BtnBackToCourseList');
