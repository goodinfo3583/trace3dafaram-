...
<tr><td style="color:#EF4444; font-weight:bold;">🚨 多殺多風險</td><td>股價↓↓ / 融資↓↓ / 融券→↓ / 借券→↓</td><td style="text-align: left;">股價下跌導致融資退場，可能出現槓桿賣壓</td></tr>
        ...
        # =========================
        # 🔗 核心隱藏按鈕：使用單一函式庫集中控制，避免重複建立衝突
        # =========================
        cols = st.columns(5)
        with cols[0]:
            if st.button("關閉", key="npc_btn_close"):
                st.session_state['show_course_npc'] = False
                st.session_state['course_view'] = 'list'
                st.rerun()
        with cols[1]:
            if st.button("第1課", key="npc_btn_c1"):
                st.session_state['course_view'] = 'detail_1'
                st.rerun()
        with cols[2]:
            if st.button("第4課", key="npc_btn_c4"):
                st.session_state['course_view'] = 'detail_4'
                st.rerun()
        with cols[3]:
            if st.button("第7課", key="npc_btn_c7"):
                st.session_state['course_view'] = 'detail_7'
                st.rerun()
        with cols[4]:
            if st.button("返回", key="npc_btn_back"):
                st.session_state['course_view'] = 'list'
                st.rerun()
...
// 尋找 Streamlit 生成的實體按鈕 (透過按鈕文字或對應位置)
const stBtns = Array.from(doc.querySelectorAll('button'));
const btnClose = stBtns.find(b => b.textContent.includes('關閉'));
const btnOpen1 = stBtns.find(b => b.textContent.includes('第1課'));
const btnOpen4 = stBtns.find(b => b.textContent.includes('第4課'));
const btnOpen7 = stBtns.find(b => b.textContent.includes('第7課'));
const btnBack = stBtns.find(b => b.textContent.includes('返回'));
