...
def inject_custom_header(is_logged_in=False):
    ...
    style.innerHTML = `
        ...
        /* 登出狀態按鈕樣式 (紅色警示) */
        .vip-logout-btn { color: #FF4C4C !important; }
    ...
    // 判斷按鈕樣式與事件
    const login_class = "vip-logout-btn" if is_logged_in else "vip-login-btn";
    const login_icon = "app/static/icon-logout.png" if is_logged_in else "app/static/icon-login.png";
    // 如果是登出，我們給他一個特殊的 id 讓他走直接重載的邏輯，而不是跳轉頁面
    const login_action_id = "direct-logout-btn" if is_logged_in else "normal-login-btn";
            ...
            <div class="system-menu">
                ...
                <div class="system-dropdown">
                    <div class="dropdown-item actionable" style="text-align: center;">
                        <!-- 💡 根據登入狀態變更按鈕樣式與 ID -->
                        <a href="#" id="${login_action_id}" data-target="登入" class="dropdown-title internal-nav ${login_class}">
                            <img src="${login_icon}" class="menu-icon" alt="login"> __LOGIN_TEXT__
                        </a>
                    </div>
    ...
    setTimeout(() => {
        ...
        navLinks.forEach(link => {
            link.onclick = (e) => {
                // 💡 處理直接登出的邏輯
                if (link.id === 'direct-logout-btn') {
                    e.preventDefault();
                    // 點擊登出按鈕，直接觸發 Streamlit 的登出 Proxy Button
                    const btns = Array.from(parentDoc.querySelectorAll('button'));
                    const targetBtn = btns.find(b => b.textContent.includes("登出"));
                    if (targetBtn) {
                        targetBtn.click();
                        // 加入小延遲後直接重整頁面，清除狀態
                        setTimeout(() => {
...
def render_proxy_buttons():
    ...
    # 處理直接登出的 callback
    def handle_logout():
        st.session_state.clear() # 清除 Session State
        st.query_params["page"] = "home" # 導回首頁
    ...
    with st.container():
        ...
        st.button("登出", on_click=handle_logout) # 修改：觸發登出函數
