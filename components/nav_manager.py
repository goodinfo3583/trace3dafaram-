...
def inject_custom_header(is_logged_in=False):
    ...
    (function() {
        ...
        if (existingHeader) {
            // 💡 如果已經存在，只更新登入狀態，絕對不重新渲染 DOM！(無閃爍核心)
            const loginBtn = existingHeader.querySelector('.vip-login-btn');
            if (loginBtn) {
                loginBtn.innerHTML = `<img src="app/static/icon-login.png" class="menu-icon" alt="login"> __LOGIN_TEXT__ <span style="color:#64748b; font-size:10px;">(Esc)</span>`;
            }
        } else {
            // 💡 第一次載入網頁時，才建立這些 DOM 元素
        ...
        // 3. 確保全域快捷鍵只綁定一次
        if (!parentWin.customHotkeyBound) {
            parentWin.customHotkeyBound = true;
            parentDoc.addEventListener('keydown', function(e) {
                ...
                if (parentWin.customHotkeyHandlerMap && parentWin.customHotkeyHandlerMap[keyStr]) {
                    e.preventDefault(); 
                    const targetName = parentWin.customHotkeyHandlerMap[keyStr];
                    const btns = Array.from(parentDoc.querySelectorAll('button'));
